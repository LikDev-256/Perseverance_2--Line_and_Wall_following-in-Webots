...
// PID Variables
 ...
 const double lKp = 0.0155;
 const double lKi = 0.0;
 const double lKd = 0.0;

 const double wKp = 0.25;
...
void computeLinePID() {
    ...
    LrightSpeed = (basespeed + LmotorVal)*1.1;
     ...
     // std::cout << "left      --     " << rightSpeed << std::endl;
      ...
      left_motor->setVelocity (boost::algorithm::clamp(LleftSpeed, 0, 10));
      right_motor->setVelocity(boost::algorithm::clamp(LrightSpeed, 0, 10));   
...
void computeWallPID() {
   ...
   // std::cout<< "WmotorVal : " << WmotorVal <<std::endl;
    ...
    WleftSpeed  = (4 + (WmotorVal)*1.1);
    WrightSpeed = (4 - (WmotorVal)*1.1);

    left_motor->setVelocity (boost::algorithm::clamp(WleftSpeed, 0, 10));
    right_motor->setVelocity(boost::algorithm::clamp(WrightSpeed, 0, 10)); 
...
void countPillar() {
    ...
    bool down2 = (rSRM < 35);
      ...
      if ((right) && (down)) {
        ...
        std::cout<< "Pillar at right side – " << rSRM-4 <<std::endl;
        right_sum1 = right_sum + (rSRM-4) * 5;

      }
      
      if ((left) && (down)) {

        std::cout<< "Pillar at left side – "  << rSLM-4 <<std::endl;
        left_sum1  = left_sum + (rSLM-4) * 3;
      ...
      // cout<< "left "  << left_sum+left_sum1   <<endl;
        ...
        if (!(already_ran1) && (already_ran5) && (leftFarReading < t && leftNearReading < t && leftCenterReading < t && MiddleReading < t 
             && rightCenterReading < t && rightNearReading < t && rightFarReading < t && Left > t1 && Middle > t1 && Right > t1)) {
        ...
        if (!(already_ran7) && (already_ran6) && (leftFarReading < t && leftNearReading < t && leftCenterReading < t && MiddleReading < t 
             && rightCenterReading < t && rightNearReading < t && rightFarReading < t && Left > t1 && Middle > t1 && Right > t1)) {
